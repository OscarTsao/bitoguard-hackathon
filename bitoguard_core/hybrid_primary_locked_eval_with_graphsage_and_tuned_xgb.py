...
def build_output_paths(max_depth: int, learning_rate: float) -> tuple[Path, Path, Path]:
    lr_token = str(learning_rate).replace(".", "p")
    out_dir = ROOT / "artifacts" / "official_features" / f"hybrid_primary_locked_eval_with_graphsage_and_tuned_xgb_md{max_depth}_lr{lr_token}"
    out_dir.mkdir(parents=True, exist_ok=True)
    return out_dir, out_dir / "hybrid_primary_locked_eval_with_graphsage_and_tuned_xgb_oof.parquet", out_dir / "hybrid_primary_locked_eval_with_graphsage_and_tuned_xgb_report.json"
...
def main() -> None:
    ...
    XGB_PARAMS["max_depth"] = int(args.max_depth)
    XGB_PARAMS["learning_rate"] = float(args.learning_rate)

    out_dir, out_oof, out_report = build_output_paths(XGB_PARAMS["max_depth"], XGB_PARAMS["learning_rate"])
    ...
    print(f"[hybrid+graphsage+tuned-xgb] saved_report={out_report}", flush=True)
    print(f"[hybrid+graphsage+tuned-xgb] saved_oof={out_oof}", flush=True)
